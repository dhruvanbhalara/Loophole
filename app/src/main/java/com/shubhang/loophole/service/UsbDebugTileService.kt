...
class UsbDebugTileService : TileService() {
    ...
    override fun onStartListening() {
        ...
        listeningJob = serviceScope.launch {
            devSettings.isUsbDebuggingEnabled.collect(::renderTile)
        }
    }
    ...
    override fun onClick() {
        super.onClick()
        serviceScope.launch {
            if (devSettings.toggle(SecureSetting.USB_DEBUGGING) is SettingsWriteResult.PermissionDenied) {
                openApp()
            }
        }
    }
    ...
    private fun renderTile(enabled: Boolean) {
        ...
        tile.label = getString(if (enabled) R.string.tile_usb_label_on else R.string.tile_usb_label_off)
        tile.updateTile()
    }
